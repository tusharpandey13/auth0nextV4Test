...
-----END PRIVATE KEY-----`,
    ...
    function getMockAuthorizationServer({ tokenEndpointResponse, discoveryResponse, audience, nonce, keyPair = DEFAULT.keyPair, onParRequest } = {}) {
    ...
    describe("handler", async () => {
        ...
        describe("rolling sessions - no matching auth route", async () => {
            it("should update the session expiry if a session exists", async () => {
                ...
                const updatedSessionCookieValue = await decrypt(updatedSessionCookie.value, secret);
    ...
    describe("handleLogin", async () => {
        ...
        it("should return an error if the discovery endpoint could not be fetched", async () => {
            ...
            expect(await response.text()).toEqual("An error occured while trying to initiate the login request.");
        });
        describe("authorization parameters", async () => {
            ...
            it("should not override internal authorization parameter values", async () => {
                ...
                expect(authorizationUrl.searchParams.get("scope")).toEqual("openid profile email offline_access custom_scope");
                expect(authorizationUrl.searchParams.get("custom_param")).toEqual("from-query");
                expect(authorizationUrl.searchParams.get("audience")).toEqual("from-config");
            });
        ...
        describe("with pushed authorization requests", async () => {
            ...
            it("should redirect to the authorization server with the request_uri and store the transaction state", async () => {
                ...
                const authClient = new AuthClient({
                    ...
                    fetch: getMockAuthorizationServer({
                        onParRequest: async (request) => {
                            ...
                            expect(params.get("redirect_uri")).toEqual(`${DEFAULT.appBaseUrl}/auth/callback`);
                            ...
                            expect(params.get("scope")).toEqual("openid profile email offline_access");
                        }
                    })
                });
                const request = new NextRequest(new URL("/auth/login", DEFAULT.appBaseUrl), {
                ...
                expect(authorizationUrl.searchParams.get("request_uri")).toEqual(DEFAULT.requestUri);
                expect(authorizationUrl.searchParams.get("client_id")).toEqual(DEFAULT.clientId);
                ...
                expect(authorizationUrl.searchParams.get("code_challenge_method")).toBeNull();
        ...
        describe("with custom callback route", async () => {
            it("should redirect to the custom callback route after login", async () => {
                ...
                expect(authorizationUrl.origin).toEqual(`https://${DEFAULT.domain}`);
                // query parameters
                expect(authorizationUrl.searchParams.get("redirect_uri")).toEqual(`${DEFAULT.appBaseUrl}/custom-callback`);
            });
        });
    });
    describe("handleLogout", async () => {
        it("should redirect to the authorization server logout URL with the correct params", async () => {
            ...
            // query parameters
            expect(authorizationUrl.searchParams.get("client_id")).toEqual(DEFAULT.clientId);
            expect(authorizationUrl.searchParams.get("post_logout_redirect_uri")).toEqual(`${DEFAULT.appBaseUrl}`);
            expect(authorizationUrl.searchParams.get("logout_hint")).toEqual(DEFAULT.sid);
        ...
        it("should use the returnTo URL as the post_logout_redirect_uri if provided", async () => {
            ...
            // query parameters
            expect(authorizationUrl.searchParams.get("client_id")).toEqual(DEFAULT.clientId);
            expect(authorizationUrl.searchParams.get("post_logout_redirect_uri")).toEqual(`${DEFAULT.appBaseUrl}/some-other-page`);
            expect(authorizationUrl.searchParams.get("logout_hint")).toEqual(DEFAULT.sid);
            // session cookie is cleared
            const cookie = response.cookies.get("__session");
            expect(cookie?.value).toEqual("");
        ...
        it("should return an error if the discovery endpoint could not be fetched", async () => {
            ...
            const response = await authClient.handleLogout(request);
            expect(response.status).toEqual(500);
            expect(await response.text()).toEqual("An error occured while trying to initiate the logout request.");
        });
    });
    describe("handleProfile", async () => {
        it("should return the user attributes stored in the session", async () => {
            ...
            const headers = new Headers();
            headers.append("cookie", `__session=${sessionCookie}`);
            const request = new NextRequest(new URL("/auth/profile", DEFAULT.appBaseUrl), {
                method: "GET",
                headers
            });
    ...
    describe("handleCallback", async () => {
        it("should establish a session — happy path", async () => {
            ...
            headers.set("cookie", `__txn_${state}=${await encrypt(transactionState, secret)}`);
            const request = new NextRequest(url, {
                method: "GET",
                headers
            });
            ...
            expect(transactionCookie.expires).toEqual(new Date("1970-01-01T00:00:00.000Z"));
        });
        it("must use private_key_jwt when a clientAssertionSigningKey is specified", async () => {
            ...
            const clientAssertionSigningKey = await crypto.subtle.importKey("pkcs8", pemToArrayBuffer(DEFAULT.clientAssertionSigningKey), {
        ...
        it("should return an error if the transaction state could not be found", async () => {
            ...
            headers.set("cookie", `__txn_does-not-exist=${await encrypt(transactionState, secret)}`);
            const request = new NextRequest(url, {
                method: "GET",
                headers
            });
        ...
        it("should return an error when there is an error authorizing the user", async () => {
            ...
            const response = await authClient.handleCallback(request);
            expect(response.status).toEqual(500);
            expect(await response.text()).toEqual("An error occured during the authorization flow.");
        });
        it("should return an error if there was an error during the code exchange", async () => {
            ...
            const response = await authClient.handleCallback(request);
            expect(response.status).toEqual(500);
            expect(await response.text()).toEqual("An error occured while trying to exchange the authorization code.");
        });
        it("should return an error if the discovery endpoint could not be fetched", async () => {
            const state = "transaction-state";
            const code = "auth-code";
            ...
            const response = await authClient.handleCallback(request);
            expect(response.status).toEqual(500);
            expect(await response.text()).toEqual("Discovery failed for the OpenID Connect configuration.");
        });
        describe("onCallback hook", async () => {
            it("should be called with the session data if the session is established", async () => {
                ...
                const mockOnCallback = vi
                    .fn()
                    .mockResolvedValue(NextResponse.redirect(new URL("/other-path", DEFAULT.appBaseUrl)));
                const secret = await generateSecret(32);
                const transactionStore = new TransactionStore({
                    secret
                });
                ...
                expect(mockOnCallback).toHaveBeenCalledWith(null, expectedContext, expectedSession);
                // validate the session cookie
                const sessionCookie = response.cookies.get("__session");
                expect(sessionCookie).toBeDefined();
                const session = await decrypt(sessionCookie.value, secret);
            ...
            it("should be called with an error if the state parameter is missing", async () => {
                ...
                const mockOnCallback = vi
                    .fn()
                    .mockResolvedValue(NextResponse.redirect(new URL("/error-page", DEFAULT.appBaseUrl)));
                const secret = await generateSecret(32);
                const transactionStore = new TransactionStore({
                    secret
                });
            ...
            it("should be called with an error if the transaction state could not be found", async () => {
                ...
                headers.set("cookie", `__txn_non-existent-state=${await encrypt(transactionState, secret)}`);
                const request = new NextRequest(url, {
                    method: "GET",
                    headers
                });
            ...
            it("should be called with an error when there is an error authorizing the user", async () => {
                ...
                const redirectUrl = new URL(response.headers.get("Location"));
                expect(redirectUrl.pathname).toEqual("/error-page");
                expect(mockOnCallback).toHaveBeenCalledWith(expect.any(Error), {
                    returnTo: transactionState.returnTo
                }, null);
                expect(mockOnCallback.mock.calls[0][0].code).toEqual("authorization_error");
                // validate the session cookie has not been set
                const sessionCookie = response.cookies.get("__session");
                expect(sessionCookie).toBeUndefined();
            });
            it("should be called with an error if there was an error during the code exchange", async () => {
                ...
                expect(mockOnCallback.mock.calls[0][0].code).toEqual("authorization_code_grant_error");
                // validate the session cookie has not been set
                const sessionCookie = response.cookies.get("__session");
                expect(sessionCookie).toBeUndefined();
            });
    ...
    describe("handleAccessToken", async () => {
        it("should return the access token if the user has a session", async () => {
            ...
            const headers = new Headers();
            headers.append("cookie", `__session=${sessionCookie}`);
            const request = new NextRequest(new URL("/auth/access-token", DEFAULT.appBaseUrl), {
                method: "GET",
                headers
            });
            ...
            const updatedSessionCookie = response.cookies.get("__session");
            const updatedSession = await decrypt(updatedSessionCookie.value, secret);
        ...
        it("should return an error if obtaining a token set failed", async () => {
            ...
            expect(await response.json()).toEqual({
                error: {
                    message: "The access token has expired and a refresh token was not provided. The user needs to re-authenticate.",
    ...
    describe("handleBackChannelLogout", async () => {
        it("should return a 204 when successful — happy path", async () => {
            ...
            const request = new NextRequest(new URL("/auth/backchannel-logout", DEFAULT.appBaseUrl), {
                method: "POST",
        ...
        it("should return a 500 if a session store is not configured", async () => {
            ...
            const response = await authClient.handleBackChannelLogout(request);
            expect(response.status).toEqual(500);
            expect(await response.text()).toEqual("A session data store is not configured.");
        });
        it("should return a 500 if a session store deleteByLogoutToken method is not implemented", async () => {
            ...
            const response = await authClient.handleBackChannelLogout(request);
            expect(response.status).toEqual(500);
            expect(await response.text()).toEqual("Back-channel logout is not supported by the session data store.");
        });
                ...
                const request = new NextRequest(new URL("/auth/backchannel-logout", DEFAULT.appBaseUrl), {
                    ...
                    body: new URLSearchParams({
                        logout_token: await generateLogoutToken({
                            claims: {
                                events: {
                                    "http://schemas.openid.net/event/backchannel-logout": "some string"
                                }
                            }
                        })
                    })
    ...
    describe("getFederatedConnectionTokenSet", async () => {
        it("should call for an access token when no federated connection token set in the session", async () => {
            ...
            const response = await authClient.getFederatedConnectionTokenSet(tokenSet, undefined, { connection: "google-oauth2", login_hint: "000100123" });
        ...
        it("should return access token from the session when federated connection token set in the session is not expired", async () => {
            ...
            const response = await authClient.getFederatedConnectionTokenSet(tokenSet, { connection: 'google-oauth2', accessToken: 'fc_at', expiresAt: Math.floor(Date.now() / 1000) + 86400 }, { connection: "google-oauth2", login_hint: "000100123" });
            const [error, federatedConnectionTokenSet] = response;
            expect(error).toBe(null);
            expect(federatedConnectionTokenSet).toEqual({
                accessToken: 'fc_at',
        ...
        it("should call for an access token when federated connection token set in the session is expired", async () => {
            ...
            const response = await authClient.getFederatedConnectionTokenSet(tokenSet, { connection: 'google-oauth2', accessToken: 'fc_at', expiresAt }, { connection: "google-oauth2", login_hint: "000100123" });
            const [error, federatedConnectionTokenSet] = response;
            expect(error).toBe(null);
            expect(federatedConnectionTokenSet).toEqual({
                accessToken: DEFAULT.accessToken,
        ...
        it("should return an error if the discovery endpoint could not be fetched", async () => {
            ...
            const [error, federatedConnectionTokenSet] = await authClient.getFederatedConnectionTokenSet(tokenSet, undefined, {
                connection: "google-oauth2"
            });
            expect(error?.code).toEqual("discovery_error");
