...
import { AccessTokenError, AccessTokenErrorCode, AuthorizationCodeGrantError, AuthorizationError, BackchannelLogoutError, DiscoveryError, FederatedConnectionAccessTokenErrorCode, FederatedConnectionsAccessTokenError, InvalidStateError, MissingStateError, OAuth2Error } from "../errors";
...
export class AuthClient {
    ...
    async defaultOnCallback(error, ctx) {
        ...
        const res = NextResponse.redirect(new URL(ctx.returnTo || "/", this.appBaseUrl));
        return res;
    }
     ...
     */
    async getFederatedConnectionTokenSet(tokenSet, federatedConnectionTokenSet, options) {
